...
class CNCClientApp:
    ...
    def create_widgets(self):
        ...
        # Bind the press event to the new unified function
    ...
    def on_write_button_press(self, event):
        """
        Starts a single thread to first write all DWORD values, then
        set the trigger boolean to TRUE, wait briefly, and set it to FALSE.
        This ensures the signal is seen by the PLC.
        """
        if not self.is_connected_var.get():
            print("WARNING: Please connect to the server first.")
            return
        ...
        def task():
            # Tüm DWORD değerleri senkron bir şekilde yazar.
            ...
            try:
                ...
                # Sinyalin PLC tarafından algılandığından emin olmak için kısa bir süre bekler
                time.sleep(0.1)
                
                # Son olarak boolean sinyalini FALSE olarak gönderir
