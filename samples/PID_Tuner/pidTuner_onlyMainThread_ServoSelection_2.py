...
class CNCClientApp:
    ...
    def create_widgets(self):
        ...
        # --- DEĞİŞTİRİLDİ: Buton davranışı güncellendi. ---
        # Eski bind komutları yerine, doğrudan yeni fonksiyonu çağıran command eklendi.
        # Bu buton artık PLC ile iletişim kurmuyor.
        self.positive_tune_button.config(command=lambda: self.capture_axis_position(self.positive_tune_value_var))
        # self.positive_tune_button.bind("<ButtonPress-1>", lambda e: self.on_tune_button_press(325, 280, self.positive_tune_value_var))
        # self.positive_tune_button.bind("<ButtonRelease-1>", lambda e: self._set_bool_value(325, False))
        ...
        # Eski bind komutları yerine, doğrudan yeni fonksiyonu çağıran command eklendi.
        # Bu buton artık PLC ile iletişim kurmuyor.
        self.negative_tune_button.config(command=lambda: self.capture_axis_position(self.negative_tune_value_var))
        # self.negative_tune_button.bind("<ButtonPress-1>", lambda e: self.on_tune_button_press(326, 281, self.negative_tune_value_var))
        # self.negative_tune_button.bind("<ButtonRelease-1>", lambda e: self._set_bool_value(326, False))
    ...
    # YENİ: Servo sürücü seçim kutusundan bir öğe seçildiğinde bu fonksiyon çalışır
    def on_servo_driver_select(self, event=None):
        ...
        # Seçenekleri ve karşılık gelen byte değerlerini bir sözlükte tanımlıyoruz
        driver_map = {
            "SMB": 1,
            "Yaskawa": 2,
            "inovance": 3,
        ...
        if value_to_write is not None:
            # DİKKAT: Bu adres projenizdeki PLC adresine göre değiştirilmelidir.
            # Örnek olarak 151 adresi kullanılmıştır.
            target_address = 0
            try:
                # set_plc_dword kullanarak integer (1 byte'lık) değeri gönderiyoruz
    ...
    def toggle_enable(self):
        ...
        try:
            # Bool değerini adrese gönder
            self.client.set_plc_bool(0, new_state)
            print(f"INFO: Set bool address 0 to {new_state}")

            # Başarılı olursa, değişkeni ve buton görünümünü güncelle
    ...
    def on_read_all_press(self):
        ...
        try:
            ...
            self.root.after(50, self._read_dword_step, 0)
        except (SendError, ConnectionError) as e:
    ...
    # --- YENİ FONKSİYON: İsteğiniz doğrultusunda eklendi ---
    def capture_axis_position(self, target_var):
        """
        Seçili eksenin o anki pozisyonunu yakalar ve hedef etikete yazar.
        Bu fonksiyon PLC'den bir değer okumaz, doğrudan arayüzdeki değeri alır.
        """
        try:
            # "Eksen Seçimi" kutusundan girilen indeksi al
            axis_index = int(self.axis_selection_var.get())
            
            # İndeksin geçerli bir aralıkta olup olmadığını kontrol et (0-5)
            if 0 <= axis_index < len(self.axis_value_vars):
                # İlgili eksenin anlık pozisyon değerini al
                current_position = self.axis_value_vars[axis_index].get()
                # Hedef etiketin değişkenini bu değerle güncelle
                target_var.set(current_position)
                print(f"INFO: Eksen {axis_index} için pozisyon yakalandı: '{current_position}'.")
            else:
                # Kullanıcı geçerli aralık dışında bir değer girdiyse uyar
            ...
            messagebox.showerror("Hatalı Giriş", "Lütfen Eksen Seçimi için geçerli bir sayı girin.")


    # --- SİLİNDİ: Bu fonksiyonlar artık yeni yöntemle gereksiz hale geldi ---
    # def on_tune_button_press(self, bool_address, read_address, value_var):
    #     """Tune pozisyonunu okuma sekansını başlatır."""
    #     if not self.is_connected_var.get() or self.is_busy: return
    #
    #     self.is_busy = True
    #     print(f"INFO: Starting 'Tune Position' read for bool_addr {bool_address}.")
    #
    #     try:
    #         self.client.set_plc_bool(bool_address, True)
    #         self.root.after(100, self._read_tune_position, read_address, value_var)
    #     except (SendError, ConnectionError) as e:
    #         self.handle_api_error(e, "in on_tune_button_press")
    #         self.is_busy = False
    #
    # def _read_tune_position(self, read_address, value_var):
    #     """Tune pozisyon değerini okur ve GUI'yi günceller."""
    #     if not self.is_connected_var.get():
    #         self.is_busy = False
    #         return
    #
    ...
    #         value_var.set(f"{real_value:.3f}")
    #         print(f"INFO: Read tune position from {read_address} -> {real_value:.3f}")
    #     except (ApiError, ConnectionError) as e:
    #         self.handle_api_error(e, "reading tune position")
    #     finally:
    #         self.is_busy = False
    # --------------------------------------------------------------------
    ...
    def check_connection_status(self):
        ...
        if self.is_connected_var.get() != current_status:
            ...
            # Bağlantı koptuğunda Enable butonunu sıfırla
    ...
    def update_gui_state(self):
        ...
        widget_list = [self.write_axis_button, self.read_all_button, self.read_clear_all_button,
                       ...
                       # YENİ: Combobox'ı da listeye ekliyoruz ki bağlantı durumuna göre aktif/pasif olsun
    ...
    # --- Yardımcı ve Değiştirilmemiş Fonksiyonlar ---
    ...
    def on_closing(self):
        ...
        if self.client and self.client.is_connected():
            # Kapatırken enable durumunu sıfırla
            try:
